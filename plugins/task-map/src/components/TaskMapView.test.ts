...
describe('TaskMapView empty states', () => {
  it('says no Project is selected and draws no cards', () => {
    ...
    expect(screen.getByRole('button', { name: 'Edit bands' })).toHaveProperty('disabled', true)
...
describe('TaskMapView dependency arrows', () => {
  ...
  it('draws the arrow into each copy of a waiter drawn in two Bands', async () => {
    renderView(
      [
        buildSeededTask({ id: 'T-1', title: 'Rotate the tokens' }),
        buildSeededTask({ id: 'T-2', title: 'Split the reader', dependsOn: ['T-1'] }),
      ...
      FIXTURE_PROJECT_ID,
      [buildLabelAssignment('T-2', 'auth', 'api')],
    )
    await screen.findByRole('button', { name: /Rotate the tokens/ })

    expect(bandsOfCard('T-2')).toEqual(['api', 'auth'])
    expect(arrowKeys()).toEqual(['T-1->T-2', 'T-1->T-2'])
  })
