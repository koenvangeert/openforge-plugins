...
export function useIssuesCreateDialog(
  ...
  async function createIssue(title: string, body: string, labels: string[]): Promise<void> {
    if (await issues.createIssue(title, body, labels)) close()
  }

  async function refineTicketDraft(
    request: Omit<RefineTicketRequest, 'projectId' | 'repo' | 'repoLabels'>,
  ): Promise<TicketDraft> {
    return issues.refineTicketDraft(request)
  }
...
export type IssuesCreateDialogController = ReturnType<typeof useIssuesCreateDialog>
