...
const boardWithout11: IssuesBoard = { ...board, issues: [issue(10, 'Ten'), issue(12, 'Twelve')] }

afterEach(() => {
  vi.clearAllMocks()
})

describe('IssuesView drawer group navigation', () => {
  ...
  it('advances to the next issue when the open one is closed rather than closing the drawer', async () => {
    let closed = false
    renderIssuesView({
      issues_get_board: async () => (closed ? boardWithout11 : board),
