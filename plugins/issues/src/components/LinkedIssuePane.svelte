<script lang="ts">
  ...
  import { loadIssueTaskLinkForTask } from '../lib/issueActions'
  ...
  function resolveTitle(issue: Issue | undefined, fallback: string | null): string | null {
  ...
  async function loadLinkedTicket(pid: string | null, tid: string) {
    ...
    loading = true
    try {
      const match = await loadIssueTaskLinkForTask(api, pid, tid)
      if (!match) return
...
<section class="flex flex-col gap-3 text-sm">
  <div>
    <h3 class="font-semibold m-0">Linked issue</h3>
  ...
  {:else if linkedTicket}
    <div class="rounded-box border border-base-300 p-3 flex flex-col gap-2">
      <div class="flex items-start gap-2">
        ...
        <div class="min-w-0">
          <p class="font-medium m-0 break-words">{linkedTicket.title ?? 'Linked issue'}</p>
  ...
  {:else}
    <p class="text-base-content/60 m-0">No issue is linked to this task.</p>
  {/if}
</section>
