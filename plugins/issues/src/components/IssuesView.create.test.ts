...
import type { IssuesBoard, Issue } from '../lib/types'
...
describe('IssuesView issue creation', () => {
  it('keeps the newly created card visible without immediately refetching the eventually-consistent board', async () => {
    ...
    const { invoke } = renderIssuesView({
      issues_get_board: async () => staleBoard,
    ...
    // fetch. Only the initial load should have hit issues_get_board.
    const boardFetches = invoke.mock.calls.filter(([method]) => method === 'issues_get_board').length
