<script lang="ts">
  ...
  interface Props {
    ...
    /** Enter on an item, and double-click, call this. Null = inert (browsing only). */
    onActivate?: ((injectable: Injectable) => void) | null
    /** Rendered in the detail footer when not editing. Receives the selected injectable. */
    detailFooter?: SvelteSnippet<[Injectable]>
    /** Called when Escape is not consumed internally. Null = ignore. */
    onEscape?: (() => void) | null
    /**
     * Land on a populated reading pane by selecting the first item once per load.
     * True for the rail view, which is a browsing surface you arrive at; false for the
  ...
  /**
   * Per-summon reset, called by the host surface (the picker on open, the view on
   * project change): reload the catalog and start fresh — nothing selected, preview
   * closed, rendered (md) view.
   */
  ...
  // Own the load lifecycle rather than relying on the host to trigger it: a host's
  // `bind:this` is not guaranteed to be assigned before its own effects run, so a
  ...
  // Auto-select once per load, never again, so closing the preview stays closed.
  let autoSelectedFor: string | null | undefined = undefined
  $effect(() => {
    if (!autoSelectFirst) return
    const first = catalog.injectables[0]
    if (!first || autoSelectedFor === loadedProjectId) return
  ...
  async function persistScope(id: string, next: { allProjects: boolean; projectIds: string[] }) {
    ...
    actionError = null
    try {
      await api.backend.invoke(METHOD.updateSnippet, {
        id,
        name: raw.name,
        body: raw.body,
        allProjects: next.allProjects,
        projectIds: $state.snapshot(next.projectIds),
      })
  ...
  async function saveCreate() {
    ...
    try {
      ...
      const created = await api.backend.invoke<Snippet>(METHOD.createSnippet, {
        name: editName,
        body: editDraft,
        allProjects: true,
        projectIds: [],
  ...
  async function saveSnippetEdit(sel: Injectable) {
    ...
    try {
      await api.backend.invoke(METHOD.updateSnippet, {
        ...
        projectIds: raw ? $state.snapshot(raw.projectIds) : [],
      })
      await catalog.reload()
  ...
  async function saveSkillEdit(sel: Injectable) {
    ...
    try {
      ...
      await api.backend.invoke(METHOD.saveSkillContent, {
        projectId,
        name: sel.name,
        level: 'user',
        sourceDir,
  ...
  async function confirmDelete() {
    ...
    if (sel.kind === 'snippet') {
      const id = snippetDbId(sel)
      if (!id) return
      performDelete = () => api.backend.invoke(METHOD.deleteSnippet, { id })
    } else {
      if (!sel.sourceDir || !sel.sourcePath) return
      const { sourceDir, sourcePath, name } = sel
      performDelete = async () => {
        await api.backend.whenReady()
        await api.backend.invoke(METHOD.deleteSkill, {
          projectId,
          name,
          level: 'user',
          sourceDir,
  ...
  // Wired into the host surface's keydown (Modal's onKeydown for the picker, the page
  // container for the view): return true to signal "handled" so the host skips its own
  // default handling. Up/down move the selection (and open the preview); Enter activates.
  ...
  export function handleKeydown(e: KeyboardEvent): boolean | void {
    ...
    // Escape outside the editor belongs to the host surface (the picker closes; the
    // full-page view ignores it).
    if (e.key === 'Escape') {
      if (!onEscape) return
      e.preventDefault()
      onEscape()
    ...
    if (e.key === 'Enter') {
      // On a group header, Enter toggles collapse (navigation, always available); on an
      // item, it activates — only when the host surface wired up an activation handler.
  ...
  // Render the confirm-delete overlay at <body> instead of inline. When this browser is
  // mounted inside the picker's modal, daisyUI's `.modal-box` sets `translate`/`scale`,
  // which establish a containing block for `position: fixed` descendants — so a nested
  // overlay would be scoped to (and clipped by) that box instead of the viewport. Safe in
