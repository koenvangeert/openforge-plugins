<script lang="ts">
  ...
  import type { Injectable, InjectableSection, Snippet } from './lib/injectableDomain'
  import {
    searchInjectables,
    filterInjectables,
    groupInjectables,
  ...
  async function saveSnippet() {
    ...
    try {
      if (mode === 'edit-snippet' && selected) {
        ...
        if (id) await api.backend.invoke(METHOD.updateSnippet, { id, name: snippetName, body: snippetBody, allProjects: snippetAllProjects, projectIds })
      } else {
        const created = await api.backend.invoke<Snippet>(METHOD.createSnippet, { name: snippetName, body: snippetBody, allProjects: snippetAllProjects, projectIds })
  ...
  async function confirmDelete() {
    ...
    try {
      if (canEditSnippet) {
        const id = snippetDbId(selected)
        if (id) await api.backend.invoke(METHOD.deleteSnippet, { id })
